...
# Mean, median, and P(final balance > $30,000) from the 100 balances in this path
final_balances = balance_path[1:]
mean_final = np.round(np.mean(final_balances), 0)
median_final = np.round(np.median(final_balances), 0)
prob_above_30k = np.round(np.mean(final_balances > 30000), 3)
print(f"Mean final balance:   ${mean_final:,.0f}")
print(f"Median final balance: ${median_final:,.0f}")
print(f"P(final balance > $30,000): {prob_above_30k}")
```
