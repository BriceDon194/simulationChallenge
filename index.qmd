...
# Only 25% of balance is gambled each round: 75% stays safe, 25% gets 1.5x (win) or 0.6x (lose)
# Effective factor per round = 0.75 + 0.25 * (1.5 or 0.6) = 1.125 (win) or 0.9 (lose)
coin_flip = np.random.binomial(n=1, p=0.5, size=n_simulations)
winning = np.where(coin_flip == 1, 1.125, 0.9)

# Cumulative balance: each period applies the 25%-at-risk factor to previous balance
balance_path = np.concatenate([[starting_balance], starting_balance * np.cumprod(winning)])
ages = np.arange(25, 76)  # 25 to 75 inclusive (51 points)
...
sim_data = pd.DataFrame({
  ...
  'age': range(25, 75),
  'coin_flip': coin_flip,
  'balance': balance_path[1:]  # balance after each sim (25% at risk: ×1.125 or ×0.9)
